...
function event_espresso_multi_price_update($event_id) {
	...
	<ul class="dynamic-price-input-form" id="dynamicPriceInput">
		<?php
            $prices = $wpdb->get_results("SELECT price_type, event_cost, surcharge, surcharge_type, currency, reservation FROM " . EVENTS_PRICES_TABLE . " WHERE event_id = '" . $event_id . "' ORDER BY id");
    ...
    $values = array(
      ...
      array('id' => 'N', 'text' => __('No', 'event_espresso'))
		    ...
		    echo '<p class="event_form_field ee_fem_form_field dynamic-price-input-field"><label class="dynamic-price-input-label" for="currency">' . __('Currency', 'event_espresso') . '</label> <input class="dynamic-price-input price-input" id="currency" type="text"  name="currency[]" value="' . $price->currency . '" /></p> ';
        echo '<p class="event_form_field ee_fem_form_field dynamic-price-input-field"><label class="dynamic-price-input-label" for="reservation">' . __('Reservation', 'event_espresso') . '</label> '.select_input("reservation", $values, $price->reservation).'</p> ';
                    echo '</p>';
		...
		<li class="dynamic-price-input-li" id="add-price-name">
			<p class="event_form_field ee_fem_form_field dynamic-price-input-field">
				...
				<input class="dynamic-price-input price-name-input" id="add-price-type-<?php echo $price_counter ?>" type="text"  name="price_type[]" value="Inversión">
			</p>
			<p class="event_form_field ee_fem_form_field dynamic-price-input-field">
				<label class="dynamic-price-input-label price" for="add-event-cost">
					<?php _e('Price', 'event_espresso'); ?>
				</label>
				<input class="dynamic-price-input price-input" id="add-event-cost" type="text" name="event_cost[]" value="0">
			</p>
			<p class="event_form_field ee_fem_form_field dynamic-price-input-field">
			...
			<p class="event_form_field ee_fem_form_field dynamic-price-input-field">
				...
				<select class="dynamic-price-input-select surcharge_type" id="add-surcharge-type" name="surcharge_type[]">
					<option value = "pct" <?php selected($org_options['surcharge_type'], 'pct') ?> selected>
                    <?php _e('Percent', 'event_espresso'); ?>
                    </option>
			...
			<p class="event_form_field ee_fem_form_field dynamic-price-input-field">
				<label class="dynamic-price-input-label" for="currency">
					<?php _e('Currency', 'event_espresso'); ?>
				</label>
				<input class="dynamic-price-input price-input" type="text" id="currency" name="currency[]" value="<?php echo $org_options['currency'] ?>" >
			</p>

      <p class="event_form_field ee_fem_form_field dynamic-price-input-field">
        <label class="selectit" for="reservation">
          <?php _e('Reservation', 'event_espresso'); ?>
        </label>
        <?php select_input("reservation", $values, $price->reservation); ?>
      </p>
...
<script type="text/javascript">
        ...
        function addPriceInput(divName){
            ...
            newdiv.innerHTML = "<p class='event_form_field ee_fem_form_field dynamic-price-input-field'>";
            newdiv.innerHTML += "<label for='add-price-type-" + (next_counter) + "'><?php _e('Name', 'event_espresso'); ?> " + (next_counter) + "</label> <input class='dynamic-price-input price-name-input' type='text' name='price_type[]' /></p> ";
            newdiv.innerHTML += "<p class='event_form_field ee_fem_form_field dynamic-price-input-field'><label for='add-price" + (next_counter) + "'><?php _e('Price', 'event_espresso'); ?></label> <input class='dynamic-price-input price-input' id='add-price-" + (next_counter) + "' type='text' name='event_cost[]' /></p> ";
            newdiv.innerHTML += "<p class='event_form_field ee_fem_form_field dynamic-price-input-field'><label for='add-surcharge-" + (next_counter) + "' ><?php _e('Surcharge', 'event_espresso'); ?></label> <input class='dynamic-price-input price-input' id='add-surcharge-" + (next_counter) + "' type='text'  name='surcharge[]' value='<?php echo $org_options['surcharge'] ?>' /></p> ";
            newdiv.innerHTML += "<p class='event_form_field ee_fem_form_field dynamic-price-input-field'><label class='dynamic-price-input-label' for='add-surcharge-type-" + (next_counter) + "'><?php _e('Surcharge Type', 'event_espresso'); ?></label> <select class='dynamic-price-input-select surcharge_type' id='add-surcharge-type-" + (next_counter) + "' name='surcharge_type[]'><option value = 'flat_rate' <?php selected($org_options['surcharge_type'], 'flat_rate') ?>><?php _e('Flat Rate', 'event_espresso'); ?></option><option value = 'pct' <?php selected($org_options['surcharge_type'], 'pct') ?> selected><?php _e('Percent', 'event_espresso'); ?></option></select></p> ";
            newdiv.innerHTML += "<p class='event_form_field ee_fem_form_field dynamic-price-input-field'><label for='currency-" + (next_counter) + "' ><?php _e('Currency', 'event_espresso'); ?></label> <input class='dynamic-price-input price-input' id='currency-" + (next_counter) + "' type='text'  name='currency[]' value='<?php echo $org_options['currency'] ?>' /></p>";
            newdiv.innerHTML += "<p class='event_form_field ee_fem_form_field dynamic-price-input-field'><label for='reservation-" + (next_counter) + "' ><?php _e('Reservation', 'event_espresso'); ?></label> '.select_input("reservation", $values, $price->reservation).' </p>";
